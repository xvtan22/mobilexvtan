...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>Private Hub chào ae<Color=/>"):Display()
    wait(.5)
...
local Window = OrionLib:MakeWindow({Name = "Private Hub", HidePremium = false, IntroText = "Private Hub⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("Private Hub SCRIPT")
W:AddLabel("DiscordPrivate Hub")
local Section = W:AddSection({
    Name = "Status"
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "Private Hub(Noitification)",
